...
DEFINE('WEBSITE_URL', 'http://my.aseshopping.az');
...
<head>
  ...
  <!-- WARNING: Respond.js doesn't work if you view the page via file:// -->
    ...
    if (empty($_POST['passport'])) { //if no name has been supplied
       ...
       if (preg_match('/[0,1]]/', $_POST['passport'])) {
        $error[] = 'Паспорт не должен содержать 0 и 1';
      }
    ...
    if (empty($_POST['pass1'])) {
      ...
      if(!preg_match('/^(?=.*\d)(?=.*[@#\-_$%^&+=§!\?])(?=.*[a-z])(?=.*[A-Z])[0-9A-Za-z@#\-_$%^&+=§!\?]{8,20}$/',$password)){
        $error[]= "Password must include: at least one lowercase char
    ...
    if (empty($error)) //send to Database if there's no error '
        ...
        if ($result_verify_email == 1) { // IF no previous user is using this email .
          ...
          if ($password == $password2) {
            ...
            $user_info['firstname'] = $name;
            $user_info['surname'] = $surname;
            $user_info['pass1'] = $password;
            ...
            $result_insert_user = registerUser($user_info);
            ...
            if ($result_insert_user == 0) {
              echo 'Query Failed ';
                ...
                if ($result_insert_user == 1) { //If the Insert Query was successfull.
                  echo '<div class="success">Thank you for registration! <br/>';
                } else { // If it did not run OK.
                  echo '<div class="errormsgbox">Регистрация не удалась из-за системной ошибки. Приносим извенения за неудобства.Попробуйте немного позже</div>';
                }
              } else {
